    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-12">
            <div className="mb-10 text-center">
                <h2 className="text-4xl font-black text-slate-950 mb-4 uppercase tracking-tight">Agenda du Parti</h2>
                        ...
                        <article key={ev.id} className="group bg-white rounded-lg border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                            ...
                            <div className="relative h-56">
                                {ev.attachment_path ? (
                                    ...
                                    <div className="w-full h-full bg-gradient-to-br from-slate-700 to-slate-950 flex items-center justify-center text-white/20 font-black text-4xl">PME</div>
                                )}
                                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-2xl text-center shadow-lg">
                                    <span className="block text-emerald-600 font-black text-xl leading-none">{new Date(ev.start_time).getDate()}</span>
                                    <span className="text-[10px] uppercase font-bold text-slate-500">{new Date(ev.start_time).toLocaleString('fr-FR', { month: 'short' })}</span>
                                </div>
                            </div>

                            <div className="p-8">
                                <h3 className="text-xl font-bold text-slate-800 mb-4 group-hover:text-emerald-600 transition-colors line-clamp-1">{ev.title}</h3>
                                ...
                                <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                                    {ev.max_attendees && (
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                                            Places : <span className="text-emerald-500">{ev.max_attendees}</span>
                                        </span>
                                    )}
                                    <Link to={`/events/${ev.id}`} className="text-sm font-black text-slate-950 group-hover:text-emerald-600 flex items-center gap-2 transition-colors uppercase tracking-wider">
                                        Parcourir <span>→</span>
                                    </Link>
                                </div>
                            </div>
